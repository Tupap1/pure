...
// Generate past 28 days (4 weeks x 7 days) mock or real data
const generateDefaultDays = (): HeatmapDay[] => {
  ...
  for (let i = 27; i >= 0; i--) {
    ...
    // Deterministic pleasant mock intensity
    const seed = (i * 7 + 3) % 5;
    const hours = seed === 0 ? 0 : Number((seed * 1.8).toFixed(1));
...
export const StudyHeatmap: React.FC<StudyHeatmapProps> = ({ days = generateDefaultDays() }) => {
  const [activeHovered, setActiveHovered] = useState<HeatmapDay | null>(days[days.length - 1] || null);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-slate-500 dark:text-slate-400">
            Registro visual de horas DME aplicadas
          </p>
        </div>
          ...
          {days.map((item, idx) => {
            ...
            return (
              <button
                ...
                } ${isHovered ? 'ring-2 ring-sky-400 scale-105 z-10' : 'hover:scale-105'}`}
              >
                ...
                {isHovered && (
                  <div className="absolute -top-9 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] font-mono px-2.5 py-1 rounded-md border border-slate-700 shadow-xl whitespace-nowrap z-30 pointer-events-none">
                    {formattedDate}: {item.hours > 0 ? `${item.hours} hrs DME` : 'Descanso'}
                  </div>
                )}
              </button>
            );
