    ...
    return (
      <div className="space-y-6 animate-pulse pb-16" role="status" aria-label="Cargando sesiones de clase">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[0, 1, 2].map((i) => (
            <div key={i} className="h-40 rounded-xl bg-slate-100 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800" />
          ))}
        </div>
      </div>
    );
                ...
                <div className="flex items-center gap-2 flex-wrap pt-1">
                  {session.notion_link && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 text-xs font-medium hover:underline transition-colors"
                    >
