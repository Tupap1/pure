...
export const CommandCenter: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {subjects.length === 0 ? (
        <Card className="p-12 text-center border-dashed border-slate-300 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/40 space-y-4">
          <GraduationCap className="w-12 h-12 text-sky-600 dark:text-sky-400 mx-auto" />
          <div>
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">Comienza a configurar tu semestre</h3>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="space-y-2 p-4">
              ...
              <div className="text-2xl font-bold text-slate-900 dark:text-slate-50">{netFreeTime}h / sem</div>
            ...
            <Card className="space-y-2 p-4">
              ...
              <div className="text-2xl font-bold text-slate-900 dark:text-slate-50">{totalDMEHours.toFixed(1)} h</div>
              <div className="w-full bg-slate-200 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
            ...
            {subjects.map((sub) => (
              <Card key={sub.id} className="space-y-2 p-4">
                ...
                <div className="w-full bg-slate-200 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
                  <div
                    className={`h-full ${
                      (sub.current_grade || 0) >= sub.target_grade ? 'bg-emerald-500' : 'bg-amber-500'
                    }`}
          ...
          <div className="space-y-4">
            ...
            {pendingDeliverables.length === 0 ? (
              <Card className="p-6 text-center border-dashed border-slate-300 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {pendingDeliverables.map((deliv) => {
                  ...
                  return (
                    <Card key={deliv.id} className="flex items-center justify-between p-3.5">
                      <div className="space-y-1">
                        ...
                        <h4 className="text-sm font-bold text-slate-900 dark:text-slate-100">{deliv.title}</h4>
