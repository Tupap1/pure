...
5. **Execution Module Rules** (Módulo de Ejecución — US1–US9):
   ...
   - **Plan the week without `program_week_id`**: `plan_week:preview` and `manage_routine_slots:rehearse` without an explicit week resolve via `resolvePlanningWeek`: if today is Sunday with a week starting tomorrow, that one; otherwise the week containing today; otherwise the next week that starts; otherwise `NO_ENCONTRADO` (program has no weeks or already finished). `plan_week:set_intentions` always requires an explicit `program_week_id` (use the one `preview` returns); a week that does not exist returns `NO_ENCONTRADO`.
